...
const defaultMongoUri = `${config.get("MONGODB_URI")}/scratch`;
const mongoUri = envMongoUri || defaultMongoUri;

if (!mongoUri) {
  console.error("MongoDB URI is not configured. Set MONGODB_URI in .env or config/development.json.");
  process.exit(1);
}
